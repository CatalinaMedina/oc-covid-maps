...
```{r gen-map_zip-function}
gen_map <- function(plot_data, shp, legend_label) {
...
```{r gen-map-city-function}
gen_all_map <- function(plot_data, shp, legend_label, geog_level) {
  if (geog_level == "city") {
...
```{r map-all-cases-function}
map_all_cases <- function( # Map number of COVID-19 cases mapped to OC zip codes
  map_data_list,
  geog_level,
  start_date, 
  end_date, 
  ...
  if (geog_level == "city") {
    ...
    gen_all_map(plot_data = plot_data, shp = shp, legend_label = legend_label, geog_level = geog_level)
