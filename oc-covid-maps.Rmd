...
neg_line_list <- read_csv(here("data/covid-data", "All PCR tests updated 2.22.21.csv")) %>% 
  ...
  new_deaths_tbl <- read_csv(here("data/covid-data", "2.22.21 release to UCI team.csv"),
...
shp <- readOGR(
  dsn = here("data/shape-files"), 
  verbose = FALSE,
  layer = "Zipcode_boundary_scag_2009"
)

shp_zip1 <- subset(shp, KEY_ %in% plot_data$KEY_)
shp_tidy <- tidy(shp_zip1, region = "KEY_") %>% 
  mutate(KEY_ = id) %>% 
